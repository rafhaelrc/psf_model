...
public class OntologyArtifact extends Artifact {
	...
	/**
	 * m√©todo retorna apenas o dado (Range) de uma propriedade de objetos
	 * M√©todo constru√≠do porque desta forma o dado √© buscado atrav√©s de uma pesquisa do reasoner
	 * Desta forma, encontra-se tamb√©m dados inferidos atrav√©s de regras SWRL relacionados a propriedade de objetos.
	 * @param individual (domain) da rela√ß√£o
	...
	/**
	 * m√©todo retorna apenas o dado (Range) de uma propriedade de dados
	 * M√©todo constru√≠do porque desta forma o dado √© buscado atrav√©s de uma pesquisa do reasoner
	 * Ainda n√£o consegue buscar dados inferidos, mas a ideia √© permitir.
	 * @param individual (domain) da rela√ß√£o
	...
	void isPurposeOfState(String state, OpFeedbackParam<String[]> purposes){
		...
		if(predicate.size() == 3) { // depois colocar aqui quando for predicado de relaÁ„o 
		...
		if(domainIsAnIndividual && rangeIsAnIndividual) { // If both terms are individuals in the ontology.
			...
			// Percorre todos os axiomas de relaÁıes do tipo object Property na ontologia
			for (OWLObjectPropertyAssertionAxiom op : queryEngineLayer.getOntology().getOntology()
				...
				// Ver se o primeiro termo tem alguma ligaÁ„o com algum individuo (range) que pertence a classe State.
				if(individualDomProv.asOWLNamedIndividual().getIRI().getShortForm().equalsIgnoreCase(predicate.get(1))) {
					// ver se o range È um individuo da classe state
			...
			// Percorrer axiomas 1 e 2 para ver se existe alguma relaÁ„o entre eles
			...
			for(OWLObjectPropertyAssertionAxiom axterm1 : axiomsTerm1AndState) {
				for(OWLObjectPropertyAssertionAxiom axterm2 : axiomsTerm2AndState) {
					/**
					 * 1) Existe uma relaÁ„o entre o  term1 e o term2
